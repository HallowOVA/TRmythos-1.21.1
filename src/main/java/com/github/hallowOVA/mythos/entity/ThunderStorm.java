...
public class ThunderStorm extends BarrierEntity {
    public ThunderStorm(Level level, LivingEntity entity) {
         this((EntityType) MythosEntityTypes.THUNDER_STORM.stream(), level);
        this.setOwner(entity);
        this.noCulling = false;
    }

    public static ThunderStorm create(Level level, LivingEntity owner) {
        ThunderStorm storm = MythosEntityTypes.THUNDER_STORM.get().create(level);
        storm.setPos(owner.getX(), owner.getY(), owner.getZ());
        storm.setOwner(owner);
        storm.noCulling = false;
        return storm;
    }
    ...
    public void tick() {
        ...
        if (owner != null) {
            ...
            List<ThunderStorm> blizzards = this.getLevel().getEntitiesOfClass(ThunderStorm.class, owner.getBoundingBox(), (entityData) -> {
                return entityData.getOwner() == owner && entityData != this;
            });
            if (!blizzards.isEmpty() || !owner.isAlive()) {
                this.discard();
            }
        }
