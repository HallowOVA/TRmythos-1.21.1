package com.github.hallowOVA.mythos.race.DivinityLine.FlameGod;
...
public class LesserGodOfFlameRace extends Race {
    public LesserGodOfFlameRace() {super(Difficulty.EASY);}
    ...
    public List<Race> getNextEvolutions(Player player) {
        ...
        list.add((Race)((IForgeRegistry) TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GOD_OF_FLAME));
        return list;
    }

    public @Nullable Race getDefaultEvolution(Player player) {
        return ((Race)((IForgeRegistry)TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GOD_OF_FLAME));
    }
    ...
    public double getEvolutionPercentage(Player player) {
        ...
        boolean hasRequiredSkill = SkillUtils.isSkillMastered(player, com.github.manasmods.tensura.registry.skill.ExtraSkills.FLAME_MANIPULATION.get());
        if (!hasRequiredSkill)
            return 0.0D;
        if (hasRequiredSkill)
            percentage += 25.0D;
    ...
    public List<Component> getRequirementsForRendering(Player player) {
        ...
        list.add(Component.translatable("trmythos.flame_manipulation.mastered"));
