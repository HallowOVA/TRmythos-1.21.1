package com.github.hallowOVA.mythos.race.DivinityLine.WindGod;
...
public class LesserGodOfWindRace extends Race {
    public LesserGodOfWindRace() {super(Difficulty.EASY);}
    ...
    public List<Race> getNextEvolutions(Player player) {
        ...
        list.add((Race)((IForgeRegistry) TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GOD_OF_WIND));
        return list;
    }

    public @Nullable Race getDefaultEvolution(Player player) {
        return ((Race)((IForgeRegistry)TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GOD_OF_WIND));
    }
    ...
    public double getEvolutionPercentage(Player player) {
        ...
        boolean hasRequiredSkill = SkillUtils.isSkillMastered(player, com.github.manasmods.tensura.registry.skill.ExtraSkills.WIND_MANIPULATION.get());
        if (!hasRequiredSkill)
            return 0.0D;
        if (hasRequiredSkill)
            percentage += 25.0D;
    ...
    public List<Component> getRequirementsForRendering(Player player) {
        ...
        list.add(Component.translatable("trmythos.wind_manipulation.mastered"));
