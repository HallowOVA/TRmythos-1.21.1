package com.github.hallowOVA.mythos.race.DivinityLine.DarknessGod;
...
public class LesserGodOfDarknessRace extends Race {
    public LesserGodOfDarknessRace() {super(Difficulty.EASY);}
    ...
    public List<Race> getNextEvolutions(Player player) {
        ...
        list.add((Race)((IForgeRegistry) TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GOD_OF_DARKNESS));
        return list;
    }

    public @Nullable Race getDefaultEvolution(Player player) {
        return ((Race)((IForgeRegistry)TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GOD_OF_DARKNESS));
    }
    ...
    public double getEvolutionPercentage(Player player) {
        ...
        boolean hasRequiredSkill = SkillUtils.isSkillMastered(player, ExtraSkills.DARKNESS_MANIPULATION.get());
        if (!hasRequiredSkill)
            return 0.0D;
        if (hasRequiredSkill)
            percentage += 25.0D;
    ...
    public List<Component> getRequirementsForRendering(Player player) {
        ...
        list.add(Component.translatable("trmythos.dark_manipulation.mastered"));
