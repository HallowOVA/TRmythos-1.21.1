package com.github.hallowOVA.mythos.race.DivinityLine.SpaceGod;
...
public class LesserGodOfSpaceRace extends Race {
    public LesserGodOfSpaceRace() {super(Difficulty.EASY);}
    ...
    public List<Race> getNextEvolutions(Player player) {
        ...
        list.add((Race)((IForgeRegistry) TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GOD_OF_SPACE));
        return list;
    }

    public @Nullable Race getDefaultEvolution(Player player) {
        return ((Race)((IForgeRegistry)TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GOD_OF_SPACE));
    }
    ...
    public double getEvolutionPercentage(Player player) {
        ...
        boolean hasRequiredSkill = SkillUtils.isSkillMastered(player, com.github.manasmods.tensura.registry.skill.ExtraSkills.SPATIAL_MANIPULATION.get());
        if (!hasRequiredSkill)
            return 0.0D;
        if (hasRequiredSkill)
            percentage += 25.0D;
    ...
    public List<Component> getRequirementsForRendering(Player player) {
        ...
        list.add(Component.translatable("trmythos.space_manipulation.mastered"));
