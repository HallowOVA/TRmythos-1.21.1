package com.github.hallowOVA.mythos.race.RevenantLine;
...
public class RevenantRace extends Race {
    public RevenantRace(Difficulty difficulty) {super(difficulty);}
    public RevenantRace() {super(Difficulty.HARD);}
    ...
    private double auraMax = 500;
    private double startingMagiculeMin = 250;
    private double startingMagiculeMax = 700;
    ...
    public List<Race> getNextEvolutions(Player player) {
        ...
        list.add((Race)((IForgeRegistry) TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.WRAITH));
        list.add((Race)((IForgeRegistry)TensuraRaces.RACE_REGISTRY.get()).getValue(TensuraRaces.LESSER_DAEMON.getId()));
        return list;
    }

    public @Nullable Race getDefaultEvolution(Player player) {
        return ((Race)((IForgeRegistry) TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.WRAITH));
    }
    ...
    public void raceAbility(Player entity) {
        if (!entity.isSpectator() && !entity.isCreative()) {
            ...
            entity.getAbilities().setFlyingSpeed(0.2F);
