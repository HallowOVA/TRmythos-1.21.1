package com.github.hallowOVA.mythos.race.MetalloidRaceLine;

import com.github.manasmods.tensura.ability.TensuraSkill;
import com.github.manasmods.tensura.race.Race;
import com.github.manasmods.tensura.registry.race.TensuraRaces;
import com.github.manasmods.tensura.util.JumpPowerHelper;
import com.github.mythos.mythos.registry.race.MythosRaces;
import com.mojang.datafixers.util.Pair;
import io.github.Memoires.trmysticism.registry.skill.IntrinsicSkills;
import net.minecraft.world.entity.player.Player;

import java.util.ArrayList;
import java.util.List;

public class MetalloidExperimenterRace extends Race {
    public MetalloidExperimenterRace() {
        super(Difficulty.INTERMEDIATE);
    }

    @Override
    public double getBaseHealth() {
        return 400;
    }

    @Override
    public double getSpiritualHealthMultiplier() {
        return 5;
    }

    @Override
    public float getPlayerSize() {
        return 2;
    }

    @Override
    public double getBaseAttackDamage() {
        return 7;
    }

    @Override
    public double getBaseAttackSpeed() {
        return 3.5f;
    }

    @Override
    public double getKnockbackResistance() {
        return 0.25f;
    }

    @Override
    public double getJumpHeight() {
        return JumpPowerHelper.defaultPlayer(2);
    }

    @Override
    public double getMovementSpeed() {
        return 0.23;
    }

    @Override
    public double getSprintSpeed() {
        return 0.26;
    }

    private double auraMin = 350000;
    private double auraMax = 350000;
    private double startingMagiculeMin = 350000;
    private double startingMagiculeMax = 350000;

    @Override
    public Pair<Double, Double> getBaseAuraRange() {
        return Pair.of(auraMin, auraMax);
    }

    @Override
    public Pair<Double, Double> getBaseMagiculeRange() {
        return Pair.of(startingMagiculeMin, startingMagiculeMax);
    }

    @Override
    public List<TensuraSkill> getIntrinsicSkills(Player player) {
        List<TensuraSkill> list = new ArrayList<>();
        list.add(IntrinsicSkills.MAGISTEEL_BODY.get());
        return list;
    }

    @Override
    public double getAuraEvolutionReward() {
        return AuraEvolutionReward();
    }
    @Override
    public double getManaEvolutionReward() {
        return ManaEvolutionReward();
    }

    private double AuraEvolutionReward() {
        return 40000;
    }

    private double ManaEvolutionReward() {
        return 40000;
    }

    @Override
    public List<Race> getNextEvolutions(Player player) {
        List<Race> list = new ArrayList<>();
        list.add((Race) TensuraRaces.RACE_REGISTRY.get().getValue(MythosRaces.METALLOID_OVERSEER));
        return list;
    }
}
