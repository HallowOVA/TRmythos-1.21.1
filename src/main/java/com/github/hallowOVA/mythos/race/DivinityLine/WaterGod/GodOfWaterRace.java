...
public class GodOfWaterRace extends Race {
    public GodOfWaterRace() {super(Difficulty.EASY);}
    ...
    public List<TensuraSkill> getIntrinsicSkills(Player player) {
        ...
        list.add(ResistanceSkills.WATER_ATTACK_NULLIFICATION.get());
        list.add(ExtraSkills.MAGIC_WATER_TRANSFORM.get());
        return list;
    }

    public List<Race> getNextEvolutions(Player player) {
        List<Race> list = new ArrayList<>();
        list.add((Race)((IForgeRegistry) TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GREATER_GOD_OF_WATER));
        return list;
    }

    public @Nullable Race getDefaultEvolution(Player player) {
        return ((Race)((IForgeRegistry)TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GREATER_GOD_OF_WATER));
    }
