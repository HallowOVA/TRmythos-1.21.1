...
public class GodOfWindRace extends Race {
    public GodOfWindRace() {super(Difficulty.EASY);}
    ...
    public List<TensuraSkill> getIntrinsicSkills(Player player) {
        ...
        list.add(ResistanceSkills.WIND_ATTACK_NULLIFICATION.get());
        list.add(ExtraSkills.MAGIC_WIND_TRANSFORM.get());
        return list;
    }

    public List<Race> getNextEvolutions(Player player) {
        List<Race> list = new ArrayList<>();
        list.add((Race)((IForgeRegistry) TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GREATER_GOD_OF_WIND));
        return list;
    }

    public @Nullable Race getDefaultEvolution(Player player) {
        return ((Race)((IForgeRegistry)TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GREATER_GOD_OF_WIND));
    }
