package com.github.hallowOVA.mythos.race.GodlingLines.NorsePantheon;
...
public class UulbornRace extends Race {
    public UulbornRace() {
    ...
    public double getEvolutionPercentage(Player player) {
        ...
        if (!TensuraPlayerCapability.isDemonLordSeed(player)) {
            evolutionPercentage = 49.9;
        }
        if (TensuraPlayerCapability.isHeroEgg(player) || TensuraPlayerCapability.isDemonLordSeed(player)){
    ...
    public List<Race> getNextEvolutions(Player player) {
        ...
        list.add((Race)((IForgeRegistry) TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.AESIR));
        return list;
    }
    ...
    public List<Component> getRequirementsForRendering(Player player) {
        ...
        list.add(Component.translatable("trmythos.race.uul_born.no_egg_seed").withStyle(ChatFormatting.RED));
        return list;
    }
