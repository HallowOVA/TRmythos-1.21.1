package com.github.hallowOVA.mythos.race.DivinityLine.LightGod;
...
public class LesserGodOfLightRace extends Race {
    public LesserGodOfLightRace() {super(Difficulty.EASY);}
    ...
    public List<TensuraSkill> getIntrinsicSkills(Player player) {
        ...
        list.add(IntrinsicSkills.LIGHT_TRANSFORM.get());
    ...
    public List<Race> getNextEvolutions(Player player) {
        ...
        list.add((Race)((IForgeRegistry) TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GOD_OF_LIGHT));
        return list;
    }

    public @Nullable Race getDefaultEvolution(Player player) {
        return ((Race)((IForgeRegistry)TensuraRaces.RACE_REGISTRY.get()).getValue(MythosRaces.GOD_OF_LIGHT));
    }
    ...
    public double getEvolutionPercentage(Player player) {
        ...
        boolean hasRequiredSkill = SkillUtils.isSkillMastered(player, ExtraSkills.LIGHT_MANIPULATION.get());
        if (!hasRequiredSkill)
            return 0.0D;
        if (hasRequiredSkill)
            percentage += 25.0D;
    ...
    public List<Component> getRequirementsForRendering(Player player) {
        ...
        list.add(Component.translatable("trmythos.light_manipulation.mastered"));
