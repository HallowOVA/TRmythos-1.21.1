...
public class EmpowermentRegenerationEffect extends SkillMobEffect {
    public EmpowermentRegenerationEffect(MobEffectCategory category, int color) {super(category, color);}
    ...
    public void applyEffectTick(LivingEntity entity, int amplifier) {
        ...
        if (missingHealth > 0.0F && !MobEffectHelper.shouldCancelHeal(entity)) {
            if (entity instanceof Player player) {
                float cost = (amplifier == 0 && SkillUtils.isSkillToggled(entity, Skills.ETERNAL.get())) ? 15.0F : 25.0F;
