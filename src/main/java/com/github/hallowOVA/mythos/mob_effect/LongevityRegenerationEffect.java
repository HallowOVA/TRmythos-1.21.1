...
public class LongevityRegenerationEffect extends SkillMobEffect {
    public LongevityRegenerationEffect(MobEffectCategory category, int color) {super(category, color);}
    ...
    public void applyEffectTick(LivingEntity entity, int amplifier) {
        ...
        if (missingHealth > 0.0F && !MobEffectHelper.shouldCancelHeal(entity)) {
            if (entity instanceof Player player) {
                float cost = (amplifier == 0 && SkillUtils.isSkillToggled(entity, Skills.ARES.get()) && SkillUtils.isSkillToggled(entity, Skills.IMMORTAL.get())) ? 20.0F : 100.0F;
