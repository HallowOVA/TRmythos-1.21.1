...
public class EnergizedRegenerationEffect extends SkillMobEffect {
    public EnergizedRegenerationEffect(MobEffectCategory category, int color) {super(category, color);}
    ...
    public void applyEffectTick(LivingEntity entity, int amplifier) {
        ...
        if (missingHealth > 0.0F && !MobEffectHelper.shouldCancelHeal(entity)) {
            if (entity instanceof Player player) {
                float cost = (amplifier == 0 && SkillUtils.isSkillToggled(entity, Skills.TENACIOUS.get())) ? 30.0F : 50.0F;
